...
debug-image:
	docker build -t ${REPOSITORY}-debug -f Dockerfile.xdebug .
...
psalm: image
	...
	${RUN_PHP_CMD} ./vendor/bin/psalm --php-version=8.0
...
profiler: debug-image
	${RUN_CMD}-debug php -d memory_limit=-1 ./examples/00-crude-benchmark.php
